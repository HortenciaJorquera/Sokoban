class Game {
  ...
  drawGame() {
    ...
    for (let square of this.floor) {
      image(
        this.floorImg,
        square[1] * SQUARE_SIDE,
        square[0] * SQUARE_SIDE,
        SQUARE_SIDE,
        SQUARE_SIDE
      );
    }
    ...
    for (let storageLocation of this.storageLocations) {
      image(
        this.storageLocationImg,
        storageLocation[1] * SQUARE_SIDE,
        storageLocation[0] * SQUARE_SIDE,
        SQUARE_SIDE,
        SQUARE_SIDE
      );
    }
  ...
  nextSquare(row, col) {
    if (this.searchForArray(this.walls, [row, col]) === -1) {
      if (this.isABox(row, col) === false) return "empty";
  ...
  isABox(row, col) {
    for (let box of this.boxes) {
      if (box.row === row && box.col === col) return box;
    }
    return false;
  }
